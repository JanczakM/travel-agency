...
class TripListOptions extends React.Component {
  handleTags(tag, checked){
    ...
    if(checked) {
      tagsArray.push(tag);
      this.props.addTag(tagsArray);
    } else {
      const tagIndex = tagsArray.indexOf(tag);
      tagsArray.splice(tagIndex, 1);
      this.props.removeTag(tagsArray);
    }
  }

  handleDuration(type, value){
    this.props.changeDuration(type, parseInt(value), this.props.filters.duration.from, this.props.filters.duration.to);
  }
